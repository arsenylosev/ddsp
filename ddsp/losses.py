...
class HmmTranscriber(tfp.distributions.HiddenMarkovModel):
  ...
  def __init__(
    ...
    **kwargs,
    ...
    transitions = (hold - other) * tf.eye(n_pitches) + other * tf.ones(
      [n_pitches, n_pitches]
    )
    transitions /= tf.reduce_sum(transitions, axis=1, keepdims=True)
    transition_distribution = tfp.distributions.Categorical(probs=transitions)
    ...
    amps_loc = tf.concat(
      [tf.ones([1]) * amps_off_center, tf.ones(n_pitches - 1) * amps_on_center],
      axis=0,
    )
    amps_scale = tf.concat(
      [tf.ones([1]) * amps_off_scale, tf.ones(n_pitches - 1) * amps_on_scale],
      axis=0,
    )
...
# ------------------------------------------------------------------------------
def amp_loss(
  amp, amp_target, loss_type='L1', weights=None, log=False, amin=1e-5
):
...
class WassersteinConsistencyLoss(Loss):
  ...
  def __init__(self, weight=1.0, midi=True, **kwargs):
    """Constructor.
...
def wasserstein_distance(u_values, v_values, u_weights, v_weights, p=1.0):
  ...
  return tf.reduce_sum(deltas * tf.abs(u_cdf - v_cdf) ** p, axis=-1) ** (
    1.0 / p
  )
...
class KDEConsistencyLoss(Loss):
  ...
  def kernel_density_estimate(self, amps, freqs, scale):
    ...
    return tfd.MixtureSameFamily(
      tfd.Categorical(probs=amps_norm), tfd.Normal(loc=freqs_midi, scale=scale)
    )
...
class TWMLoss(Loss):
  ...
  def call(self, f0_candidates, freqs, amps):
    ...
    combined_loss = (
      self.sinusoids_weight * sinusoids_loss
  ...
  def get_loss_tensors(self, f0_candidates, freqs, amps):
    ...
    freq_ratios = safe_divide(
      freqs[:, :, tf.newaxis, :], f0_candidates[:, :, :, tf.newaxis]
    )
    ...
    sinusoids_loss = safe_divide(
      tf.reduce_sum(nll_sinusoids * a, axis=-1), tf.reduce_sum(a, axis=-1)
    )
    ...
    harmonics_loss = (
      nll_harmonics * amps_prior[tf.newaxis, tf.newaxis, tf.newaxis, :]
    )
    ...
    nyquist_mask = tf.where(
      harmonics < nyquist_midi,
  ...
  def get_p_sinusoids_given_harmonics(self):
    ...
    harmonics_probs = (
      tf.ones(self.n_harmonic_gaussians) / self.n_harmonic_gaussians
    )
