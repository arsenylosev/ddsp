...
class PrepareTFRecordBeamTest(parameterized.TestCase):
  ...
  def setUp(self):
    ...
    scipy.io.wavfile.write(
      ...
      np.random.randint(
        ...
        size=int(self.wav_sr * self.wav_secs),
        dtype=np.int16,
      ),
    )
  ...
  def parse_tfrecord(self, path):
    return [
      ...
      for record in tf.data.TFRecordDataset(os.path.join(self.test_dir, path))
    ]

  def validate_outputs(self, expected_num_examples, expected_feature_lengths):
    all_examples = self.parse_tfrecord(
      'output.tfrecord-00000-of-00002'
    ...
    for ex in all_examples:
      ...
      for feat, expected_len in expected_feature_lengths.items():
        ...
        try:
          ...
          raise AssertionError(f'feature: {feat}') from e
  ...
  @parameterized.named_parameters(
    ('16kHz', 16000), ('32kHz', 32000), ('48kHz', 48000)
  )
  ...
  @parameterized.named_parameters(
    ('16kHz', 16000), ('44.1kHz', 44100), ('48kHz', 48000)
  )
  ...
  def test_direct_running_mode_in_memory(self):
    """Test that direct_running_mode='in_memory' works correctly."""
    prepare_tfrecord_lib.prepare_tfrecord(
      [self.wav_path],
      os.path.join(self.test_dir, 'output.tfrecord'),
      num_shards=2,
    ...
    self.assertLen(all_examples, 1)

  def test_direct_running_mode_multithreading(self):
    """Test that direct_running_mode='multi_threading' works correctly."""
    prepare_tfrecord_lib.prepare_tfrecord(
      [self.wav_path],
      os.path.join(self.test_dir, 'output.tfrecord'),
      num_shards=2,
  ...
  def test_direct_running_mode_none_uses_default(self):
    """Test that not setting direct_running_mode works (defaults to None)."""
    prepare_tfrecord_lib.prepare_tfrecord(
      [self.wav_path],
      os.path.join(self.test_dir, 'output.tfrecord'),
      num_shards=2,
