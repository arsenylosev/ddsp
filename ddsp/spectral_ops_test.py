...
def gen_np_sinusoid(frequency, amp, sample_rate, audio_len_sec):
  """Generate a numpy sinusoid."""
  return test_util.gen_np_sinusoid(frequency, amp, sample_rate, audio_len_sec)
...
class PadOrTrimVectorToExpectedLengthTest(
...
class ComputeFeaturesTest(parameterized.TestCase, tf.test.TestCase):
  ...
  @parameterized.named_parameters(
    ('same_.21secs', 'same', 0.21),
    ('same_.4secs', 'same', 0.4),
    ('center_.21secs', 'center', 0.21),
    ('center_.4secs', 'center', 0.4),
    ('valid_.21secs', 'valid', 0.21),
    ('valid_.4secs', 'valid', 0.4),
  )
  ...
  @parameterized.named_parameters(
    ('16k_.21secs', 16000, 0.21),
    ('24k_.21secs', 24000, 0.21),
    ('44.1k_.21secs', 44100, 0.21),
    ('16k_.4secs', 16000, 0.4),
    ('24k_.4secs', 24000, 0.4),
    ('44.1k_.4secs', 44100, 0.4),
  )
  def test_compute_loudness(self, sample_rate, audio_len_sec):
    ...
    expected_len = self.expected_db_length(audio_sin, sample_rate, padding)
    loudness = spectral_ops.compute_loudness(
      audio_sin, sample_rate, self.frame_rate, self.frame_size, padding=padding
    )
