...
def get_a_weighting(sample_rate, n_fft):
    """Get cached A-weighting filter for given sample rate and FFT size."""
    key = (sample_rate, n_fft)
    if key not in _A_WEIGHTING_CACHE:
        frequencies = librosa.fft_frequencies(sr=sample_rate, n_fft=n_fft)
        a_weighting = librosa.A_weighting(frequencies)[None, None, :]
        _A_WEIGHTING_CACHE[key] = 10 ** (a_weighting / 10)
    return _A_WEIGHTING_CACHE[key]
...
def compute_rms_energy(
    audio, sample_rate=16000, frame_rate=250, frame_size=512, padding="center"
):
...
def compute_loudness(
    ...
    # Perceptual weighting (cached for efficiency).
    weighting = get_a_weighting(sample_rate, n_fft)
    power = power * weighting

    # Average over frequencies (weighted power per a bin).
    avg_power = reduce_mean(power, axis=-1)
    loudness = core.power_to_db(
        avg_power, ref_db=ref_db, range_db=range_db, use_tf=use_tf
    )
...
def pad_or_trim_to_expected_length(
    vector, expected_len, pad_value=0, len_tolerance=20, use_tf=False
):
    ...
    if abs(vector_len - expected_len) > len_tolerance:
        ...
        raise ValueError(
            ...
            "beyond tolerance of : {}".format(vector_len, expected_len, len_tolerance)
        )
    # Pick tensorflow or numpy.
    lib = tf if use_tf else np
    ...
    if vector_len < expected_len:
        ...
        vector = lib.pad(
            vector, ((0, 0), (0, n_padding)), mode="constant", constant_values=pad_value
        )
    # Trim samples
...
class PretrainedCREPE(tf.keras.Model):
    ...
    def __init__(self, model_size_or_path, hop_size=160, **kwargs):
    ...
    def create_hmm(self, num_steps):
        ...
        initial_distribution = tfp.distributions.Categorical(probs=tf.ones([360]) / 360)
        ...
        transition_distribution = tfp.distributions.Categorical(probs=transition)

        # Emission probability = fixed probability for self, evenly distribute the
        # others.
        self_emission = 0.1
        emission = tf.eye(360) * self_emission + tf.ones(shape=(360, 360)) * (
            (1 - self_emission) / 360.0
        )
